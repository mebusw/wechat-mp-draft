...
1. 先调用SKILL `/wechat-markdown-html-render` 来分别渲染文本和代码样式，得到html格式的输出文件。
2. 调用任何可用的文生图片 image generation SKILL （优先使用`/huny-img`，然后是`wanx-img`）来配1张封面图（ar 2.35:1）
3. 继续调用本SKILL的工作流程来访问微信公众号文章草稿箱
