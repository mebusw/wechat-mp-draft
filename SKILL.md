...
1. 基于用户输入，调用SKILL `/jackyshen-write-wechat-article` 改写为微信公众号风格的文章，md格式
2. 调用SKILL `/wechat-markdown-html-render` 来分别渲染文本和代码样式，得到html格式的输出文件。
3. 调用任何可用的文生图片 image generation SKILL （优先使用`/huny-img`，然后是`wanx-img`等其他）来配1张封面图（ar 2.35:1）
4. 继续调用本SKILL的工作流程来访问微信公众号文章草稿箱
